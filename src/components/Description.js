...
const DescriptionComponent = ({content, showDesc, size}) => {
  const currentStyle = size != null ? { fontSize: `${Math.round(size)}px`, color: "red", opacity: `${(Math.round(window.pageYOffset*3) - 7150 - 4403.2001953125) / 4403.2001953125}` } : {};
  ...
  return (
    <div style={{ width: "77vw", marginTop: "-10vh", float: 'right' }}>
      ...
      style={currentStyle}>{content.description}</p> : <div>
        <a className='TwitterBtn' href="https://twitter.com/intent/tweet?text= &hashtags=HamasWarCrimes">
